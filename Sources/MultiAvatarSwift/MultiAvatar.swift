...
@available(iOS 13.0, *)
public struct Multiavatar {
    public static func generate(string: String, sansEnv: Bool = false, vpWidth: Int? = 231, vpHeight: Int? = 231, ver: [PartType: (PartVersion, ThemeVersion?)]? = nil) -> String {
        // '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 231 231">'
        //<rect x=\"0\" y=\"0\" width=\"100%\" height=\"100%\"/>"
//        let svgStart = "<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"100%\" height=\"100%\" viewBox=\"0 0 231 231\">"
        
        
        //viewBox=\"0 0 231 231\">"
        let svgStart = "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 \(vpWidth!) \(vpHeight!)\">"
        print(svgStart)
        let svgEnd = "</svg>"
...
//        print("sha256Numbers: ", sha256Numbers)
        ...
        parts[.env] = Int((multiplier * Float(hash[0 ..< 2])!).rounded())
        parts[.clo] = Int((multiplier * Float(hash[2 ..< 4])!).rounded())
        parts[.head] = Int((multiplier * Float(hash[4 ..< 6])!).rounded())
        parts[.mouth] = Int((multiplier * Float(hash[6 ..< 8])!).rounded())
        parts[.eyes] = Int((multiplier * Float(hash[8 ..< 10])!).rounded())
...
//        print("partsStr: ", partsString)
        
        var final = [PartType: String]()
        
        for part in partsString {
            var partV = part.value[0 ..< 2]
            var theme = ThemeVersion(rawValue: part.value[2...])
            
            if let forcePart = ver?[part.key] {
                partV = forcePart.0.rawValue
                if let themeVersion = forcePart.1 {
                    theme = themeVersion
                }
            }
            
            final[part.key] = getFinal(part: part.key, partV: partV, theme: theme!)
        }
        
        //        if let forcedPart = ver {
        //            for part in forcedPart {
        //                final[part.key] = getFinal(part: part.key, partV: part.value.0?.rawValue ?? PartVersion.allCases.randomElement() , theme: (part.value.1 ?? ThemeVersion.allCases.randomElement()) ?? ThemeVersion.a)
        //            }
        //        }
...
//        print("\(svgStart)\(final[.env]!)\(final[.head]!)\(final[.clo]!)\(final[.top]!)\(final[.eyes]!)\(final[.mouth]!)\(svgEnd)")
    ...
    static func getFinal(part: PartType, partV: String, theme: ThemeVersion) -> String {
...
//                print("replace: ", "\(index) \(result[index]) \(colors[index])")
                resultFinal = resultFinal.replaceFirst(of: result[index], with: "\(colors[index]);")
            }
        }
//        print("\(part): \(resultFinal)")
    ...
    private static let themes: [String: [ThemeVersion: [PartType: [String]]]] = {
        var themes: [String: [ThemeVersion: [PartType: [String]]]] = [
            // Robo
            "00": [
                .a: [
                    .env: ["#ff2f2b"],
    ...
    //    let sP: StringPartsCollection(parts: ["none": [.none : "none"]])
    
    private static let sP: StringPartsCollection = {
        // Optimization
        let env = "<path d=\"M33.83,33.83a115.5,115.5,0,1,1,0,163.34,115.49,115.49,0,0,1,0-163.34Z\" style=\"fill:#01;\"/>"
        let head = "<path d=\"m115.5 51.75a63.75 63.75 0 0 0-10.5 126.63v14.09a115.5 115.5 0 0 0-53.729 19.027 115.5 115.5 0 0 0 128.46 0 115.5 115.5 0 0 0-53.729-19.029v-14.084a63.75 63.75 0 0 0 53.25-62.881 63.75 63.75 0 0 0-63.65-63.75 63.75 63.75 0 0 0-0.09961 0z\" style=\"fill:#000;\"/>"
        let str = "stroke-linecap:round;stroke-linejoin:round;stroke-width:"
        
        var sP = StringPartsCollection(parts: ["none": [.none: "none"]])
        // Robo
        sP["00"][.env] = env
...
struct StringPartsCollection {
    typealias DictionaryType = [String: [PartType: String]]
    
    // Underlying, private storage, that is the same type of dictionary
    // that we previously was using at the call site
    private var parts = DictionaryType()
    // Enable our collection to be initialized with a dictionary
    init(parts: DictionaryType) {
        self.parts = parts
    }
}

extension StringPartsCollection: Collection {
    // Required nested types, that tell Swift what our collection contains
    typealias Index = DictionaryType.Index
    typealias Element = DictionaryType.Element
    // The upper and lower bounds of the collection, used in iterations
    var startIndex: Index { return parts.startIndex }
    var endIndex: Index { return parts.endIndex }
    // Required subscript, based on a dictionary index
    subscript(index: Index) -> StringPartsCollection.DictionaryType.Element { return parts[index] }
    
    // Method that returns the next index when iterating
    func index(after i: StringPartsCollection.DictionaryType.Index) -> StringPartsCollection.DictionaryType.Index {
        return parts.index(after: i)
    }
}

extension StringPartsCollection {
    subscript(string: String) -> [PartType: String] {
        get { return parts[string] ?? [.none: ""] }
        set { parts[string] = newValue }
    }
}
...
public enum ThemeVersion: String, CaseIterable {
    case a = "A"
    case b = "B"
    case c = "C"
}

public enum PartVersion: String, CaseIterable {
    case robo = "00"
    case girl = "01"
    case blonde = "02"
    case evilnormie = "03"
